...
s_Hikvision_DB  c_Variable::g_Hikvision_DB;
...
**Function:状态回调
*************************************************************************************************************************************************/
void c_Variable::ExceptionCallBack(DWORD dwType, LONG lUserID, LONG lHandle, void *pUser)
{
    Q_UNUSED(lUserID);
    Q_UNUSED(lHandle);
    Q_UNUSED(pUser);
    emit c_Variable::getInstance().Status(g_Hikvision_DB.device_name + "：" + c_Variable::getInstance().Hikvision_Status(dwType));
}
/*************************************************************************************************************************************************
**Function:数据解码回调函数，将YV_12格式的视频数据流转码为可供opencv处理的BGR类型的图片数据，并实时显示。
*************************************************************************************************************************************************/
void c_Variable::DecCBFun(int nPort,char * pBuf,int nSize,FRAME_INFO * pFrameInfo, void* nReserved1,int nReserved2)
{
    Q_UNUSED(nPort);Q_UNUSED(nSize);Q_UNUSED(nReserved1);Q_UNUSED(nReserved2);Q_UNUSED(pFrameInfo);
}
/*************************************************************************************************************************************************
**Function:事件回调
*************************************************************************************************************************************************/
void c_Variable::RealDataCallBack_V30(LONG lRealHandle, DWORD dwDataType, BYTE *pBuffer, DWORD dwBufSize, DWORD dwUser)
{
    Q_UNUSED(dwUser);
    if (lRealHandle == g_Hikvision_DB.lRealPlayHandle) {
		switch (dwDataType) {
		case NET_DVR_SYSHEAD: {
            if (!PlayM4_GetPort(&g_Hikvision_DB.lPort)) {
                emit c_Variable::getInstance().Status("监控相机：获取播放库未使用的通道号失败");
				break;
			}
            if (!PlayM4_SetStreamOpenMode(g_Hikvision_DB.lPort, STREAME_REALTIME)) {
                emit c_Variable::getInstance().Status("监控相机：设置实时流播放模式失败");
				break;
			}
            if (!PlayM4_OpenStream(g_Hikvision_DB.lPort, pBuffer, dwBufSize, 1024 * 1024)) {
                emit c_Variable::getInstance().Status("监控相机：打开流接口失败");
				break;
			}
            if (!PlayM4_SetDecCallBackEx(g_Hikvision_DB.lPort, DecCBFun, nullptr, 0)) {
                emit c_Variable::getInstance().Status("监控相机：设置解码回调函数(解码且显示)失败");
				break;
			}
            if (!PlayM4_Play(g_Hikvision_DB.lPort, g_Hikvision_DB.winId)) {
                emit c_Variable::getInstance().Status("监控相机：打开视屏解码失败");
				break;
			}
			else {
                emit c_Variable::getInstance().Status("监控相机：打开视屏解码成功");
				break;
			}
		}
		case NET_DVR_STREAMDATA: {
            if (dwBufSize > 0 && g_Hikvision_DB.lPort != -1) {
                if (!PlayM4_InputData(g_Hikvision_DB.lPort, pBuffer, dwBufSize)) {
                    emit c_Variable::getInstance().Status("监控相机：码流数据获取失败");
				}
			}
			break;
		}
		default: {
            emit c_Variable::getInstance().Status("监控相机：未知的数据类型");
			break;
		}
		}
	}
